...
As the first paper provided the basic pseudo-code of BO, we will define a function to optimize using BO and R built-in function `optimize` and compare the results. We define the following function
$$
...
In this setup, GP model includes only an intercept term set to zero. The covariance function is defined as the exponential of the negative squared Euclidean distance, i.e., $\exp{-(x - x')^2}$ for all $x, x' \in \mathcal{X}$. We assume noise-free evaluations, meaning that the objective function is observed without measurement error.
